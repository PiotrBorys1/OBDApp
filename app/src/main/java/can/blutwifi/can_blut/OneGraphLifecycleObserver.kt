...
@SuppressLint("Range")
class OneGraphLifecycleObserver(private val registry : ActivityResultRegistry, private val activity1: OneGraphActivity)
        ...
        {uri->
            if (uri!=null)
            {
                ...
                val outpuStraem=activity1.applicationContext.contentResolver.openOutputStream(uri)
                    ...
                    while (i < (MutableList(activity1.listOfValuesArraysVisible.size + 1) { s ->
                            if (s < activity1.listOfValuesArraysVisible.size) {
                                activity1.listOfValuesArraysVisible[s].size
                            } else {
                                activity1.listOfTimeValues.size
                            }
                        }).min()) {
                        if (i == -1) {
                            while (j < activity1.listOfGraphs.size) {
                                if (j == -1) {
                                    outpuStraem!!.write("Time[ms];".encodeToByteArray())
                                } else if (activity1.listOfVisiblegraphs[j]) {
                        ...
                        {
                            while (j < activity1.listOfValuesArraysVisible.size) {
                                if (j == -1) {
                                    outpuStraem!!.write((activity1.listOfTimeValues[i].toString() + ";").encodeToByteArray())
                                } else {
                                    outpuStraem!!.write((activity1.listOfValuesArraysVisible[j][i].toString() + ";").encodeToByteArray())
                                }
                ...
                activity1.handler1.obtainMessage(MESSAGE_FINISH_SAVING).sendToTarget()
            }
    ...
    fun saveFile()
    {
      ...
      /*  when {
            ContextCompat.checkSelfPermission(
                ...
                launcher_file_access_permission.launch("android.permission.WRITE_EXTERNAL_STORAGE")
            }
        }*/
